namespace BrainAI.BehaviorTrees.Composites
{
    ...
    using BrainAI.BehaviorTrees.Conditionals;
    using BrainAI.BehaviorTrees.Decorators;
    ...
    public abstract class Composite<T> : Behavior<T>
    {
        ...
        {
            return this.Children[0] is IConditional<T>;
        }
        ...
        bool HasLowerPriorityConditionalAbortInChildren()
        {
            for( var i = 0; i < this.Children.Count; i++ )
            {
                ...
                var composite = this.Children[i] as Composite<T>;
        ...
        protected void UpdateLowerPriorityAbortConditional( T context, TaskStatus statusCheck )
        {
            ...
            for( var i = 0; i < this.CurrentChildIndex; i++ )
            {
                var composite = this.Children[i] as Composite<T>;
                if( composite != null && composite.AbortType.Has( AbortTypes.LowerPriority ) )
                {
                    ...
                    if( status != statusCheck )
                    {
                        ...
                        for( var j = i; j < this.Children.Count; j++ )
                            this.Children[j].Invalidate();
                        break;
                    }
                }
            }
